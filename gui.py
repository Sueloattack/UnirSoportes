...
class ResultsWindow(customtkinter.CTkToplevel):
    """
    Ventana de resultados que vuelve al diseño estable usando CTkLabel.
    El texto no será seleccionable, pero la ventana no causará errores.
    """
...
# --- EL RESTO DE LA CLASE APP PERMANECE IGUAL ---
class App(customtkinter.CTk):
    def __init__(self):
        super().__init__()
        self.title("Unificador de Soportes para Cuentas de Cobro")
    ...
    def create_widgets(self):
        ...
        frame_select.grid(row=0, column=0, padx=20, pady=20, sticky="ew")
        ...
        self.button_process.grid(row=1, column=0, padx=20, pady=10, sticky="ew")
        frame_progress = customtkinter.CTkFrame(self, fg_color="transparent")
        frame_progress.grid(row=2, column=0, padx=20, pady=10, sticky="ew")
    ...
    def start_processing_thread(self):
        ...
        self.progress_bar.set(0)
        thread = threading.Thread(target=core_logic.process_folders, args=(self.selected_folder_path, self))
        thread.daemon = True
        thread.start()
